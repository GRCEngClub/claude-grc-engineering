...
- **Human-subjects research** (interviews, surveys, observation of people) normally requires IRB/ethics review. Researchers without a home IRB need a path. Options, in rough order of preference: redesign around public/secondary data; partner with an affiliated co-author whose institution's IRB can review (see `authorship-collaboration.md`); use a commercial/independent IRB (costs money); if the user is a student, ask whether their home institution provides IRB review for student research. Publishing human-subjects work with **no** ethics review will block most reputable venues and is an integrity problem regardless of venue.
- **Sensitive professional data**: if the user draws on employer, client, or government work, anything used in a public artifact must be public, authorized, or fully abstracted — when in doubt, treat it as off-limits and say so. Never let the convenience of insider knowledge contaminate a public research artifact.
