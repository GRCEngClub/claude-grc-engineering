...
- **Storing the JSON file in a shared directory**: `--write-to=./prod-db` (relative) is rejected — the helper only accepts absolute paths under `~/.config/claude-grc/secrets/`. This is intentional; the connector refuses to write secrets to working directories.
