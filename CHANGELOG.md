...
- **`nist-800-53` `control-tailor` misallocated two AC-2 enhancements in its Moderate baseline example.** AC-2(5) was listed under `Baseline Includes:` with a `✓` but annotated `(not baseline, but common)`, and AC-2(13) sat in the optional list. SP 800-53B allocates both to Moderate and High. Both errors understated the Moderate baseline, so a user tailoring from this output would under-scope their SSP. Dropped the parenthetical from AC-2(5) and moved AC-2(13) into `Baseline Includes:`. AC-2(6)–(9), (11), and (12) are not in the NIST Moderate baseline and stay in the optional list. Closes [#230](https://github.com/GRCEngClub/claude-grc-engineering/issues/230).
- **`gap-assessment` no longer reports a clean pass for frameworks that do not resolve in the crosswalk.** Every requested framework is now resolved against the SCF crosswalk before any report is produced; anything unresolved aborts with new exit code `7` and a per-framework reason (the specific no-public-crosswalk explanation for HITRUST/StateRAMP/PBMM/US-Export, a not-found hint otherwise), instead of rendering a green "Tier 1 blockers (0)" report with exit 0 for a framework that was never evaluated. Dropped `HITRUST-CSF` and `PBMM` from the documented alias list in `commands/gap-assessment.md` and documented the unsupported set. Closes [#215](https://github.com/GRCEngClub/claude-grc-engineering/issues/215).
