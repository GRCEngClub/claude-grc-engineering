...
- **`/nist:evidence-checklist` command.** Added a baseline-scoped (Low/Moderate/High) evidence collection checklist to the `nist-800-53` plugin, matching the `gdpr`/`hitrust`/`cmmc` evidence-checklist convention. Includes worked examples for AC-2 (Account Management) and AU-6 (Audit Record Review, Analysis, and Reporting), NIST SP 800-53A assessment-method framing, baseline-depth deltas, and a baseline-aware Python collector. Closes [#163](https://github.com/GRCEngClub/claude-grc-engineering/issues/163).
