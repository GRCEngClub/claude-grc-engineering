...
- **SCF API links moved to the club-canonical host.** Replaced the legacy `hackidle.github.io` SCF API host with `grcengclub.github.io` across all four framework-plugin scaffold templates (the root cause: every new plugin inherited the legacy host) and the five merged plugins still carrying it (`au-apra-cps-234`, `jp-appi`, `sg-mas-trm`, `us-finra`, `us-nerc-cip`). All fourteen swapped links verified returning 200 on the canonical mirror. Follows up the host correction reviewed in [#213](https://github.com/GRCEngClub/claude-grc-engineering/pull/213); historical CHANGELOG entries and LICENSE attribution intentionally untouched.
